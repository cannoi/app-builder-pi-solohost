  ...
  async runPodmanApp({ sourcePath, projectSlug, timeout = 180, keepRunning = true } = {}) {
    ...
    try {
      ...
      return {
        ...
        logs: clip(logs), e2e, internet: { enabled: true, mode: 'bridge', dns: ['1.1.1.1', '8.8.8.8'] }, keptRunning: Boolean(keepRunning), sandbox: { memoryMb: 512, cpus: 1, hostBind: '127.0.0.1' },
      };
    } catch (err) {
      await this.stopApp({ projectSlug: safeSlug, removeImage: false }).catch(() => {});
