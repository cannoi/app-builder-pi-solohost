...
export function classifyLogs(logs = '') {
  ...
  if (/fetch failed|econnrefused|couldn't connect|preview port is not open/i.test(t)) {
    return { code: 'not_listening', title: 'Preview could not open the test page.', hint: 'Tap Run again. App Builder will serve the app files directly if the generated server is not ready.' };
  }
  return null;
}
