(() => {
  ...
  function showNetError(kind) {
    if (kind === 'offline') {
      els.browserErrorTitle.textContent = 'Không có kết nối Internet';
      els.browserErrorDesc.textContent = 'Thiết bị hiện không có Internet. Kiểm tra kết nối mạng rồi thử lại.';
    } else {
      els.browserErrorTitle.textContent = 'Không thể tải trang';
      els.browserErrorDesc.textContent = 'Trang mất quá lâu để phản hồi hoặc không thể truy cập. Kiểm tra kết nối Internet rồi thử lại.';
    }
  ...
  async function ensureProxyBase() {
    ...
    try {
      ...
      if (!res.ok) throw new Error('gateway discovery failed');
      const data = await res.json();
      if (!data.base) throw new Error('gateway URL missing');
      state.proxyBase = String(data.base).replace(/\/$/, '');
      state.proxyReady = true;
      return state.proxyBase;
    } catch {
      // Safe local fallback for standard SoloHost mapping.
      const host = location.hostname || '127.0.0.1';
      state.proxyBase = `${location.protocol}//${host}:18081`;
  ...
  function frameUrl(url) {
    ...
    return `${state.proxyBase || `${location.protocol}//${location.hostname}:18081`}/proxy?url=${encodeURIComponent(url)}`;
  }
  ...
  async function showBrowser(url, record) {
    ...
    if (!navigator.onLine) {
      ...
      showNetError('offline');
    } else {
      let target = url;
      if (/^https?:\/\//i.test(url)) {
        target = `${await ensureProxyBase()}/proxy?url=${encodeURIComponent(url)}`;
      }
      hideNetError();
      els.iframe.src = target;
      state.loadTimer = setTimeout(() => {
        if (state.pendingUrl === url) showNetError('timeout');
      }, PAGE_LOAD_TIMEOUT);
    }
  ...
  window.addEventListener('online', () => {
    if (document.body.classList.contains('browsing') && state.pendingUrl) {
      showBrowser(state.pendingUrl, false);
    }
  });
